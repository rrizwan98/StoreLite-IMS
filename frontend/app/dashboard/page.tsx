...
import { useWidgetConfig, WidgetId } from './hooks/useWidgetConfig';
...
export default function DashboardPage() {
  ...
  // v1.4: Widget configuration for customizable dashboard
  const {
    config: widgetConfig,
    isLoaded: widgetConfigLoaded,
    toggleWidget,
    reorderWidgets,
    resetToDefault: resetWidgetConfig,
  } = useWidgetConfig();
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center space-x-3">
                ...
                {/* v1.4: Customize Dashboard button */}
                <button
            ...
            {/* v1.4: Render widgets in configured order */}
            {widgetConfigLoaded && widgetConfig.order.map((widgetId) => {
              // Skip hidden widgets
              if (!widgetConfig.visibility[widgetId]) return null;
              ...
              switch (widgetId) {
                ...
                case 'kpiStats':
                  return (
                    <div key={widgetId} data-tour="kpi-stats">
                      <KPIStatsRow
                ...
                case 'recentActivity':
                  return <RecentActivityPanel key={widgetId} className="mb-8" />;

                case 'featureCards':
                  return (
                    <div key={widgetId} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
                ...
                case 'connectedTools':
                  return (
                    <div key={widgetId} data-tour="connected-tools">
                      <ConnectToolsSection className="mt-0" />
                    </div>
                  );
      ...
      {/* v1.4: Customize Dashboard Modal */}
      {isSchemaQueryOnly && (
        <CustomizeDashboardModal
          ...
          onResetToDefault={resetWidgetConfig}
        />
      )}
    </div>
  );
