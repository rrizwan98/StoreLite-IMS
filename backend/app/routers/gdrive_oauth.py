...
async def gdrive_oauth_callback(
    ...
    """
    logger.info(f"[GDrive] OAuth callback, state={state[:8]}...")
...
async def save_gdrive_connector(
    ...
    # Connector name with email
    email = user_info.get("email", "")
    connector_name = f"Google Drive - {email}" if email else "Google Drive"
...
async def disconnect_gdrive(
    ...
    logger.info(f"[GDrive] Disconnected for user {current_user.id}")
